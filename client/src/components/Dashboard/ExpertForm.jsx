...
function ExpertForm(props) {
  ...
  const [clickEchauffement, setClickEchauffement] = useState(
    props.seance.echauffements.length > 0 ? true : false
  );
  const [paramsSelect, setParamsSelect] = useState();
  const [clickExercices, setClickExercices] = useState(
    props.seance.exercices.length > 0 ? true : false
  );
  const [clickDetails, setClickDetails] = useState(
    props.seance.details.length > 0 ? true : false
  );
    ...
    if (err === false) {
      ...
      if (searchParams.get('seanceId')) {
        data = await API.debutantform({
          seance: { ...seance, id: searchParams.get('seanceId') },
  ...
  function onAddExercices(event) {
    ...
    newS = {
      ...
      exercices: [
        ...newS.exercices,
        { exercice: { name: '' }, Series: {}, Categories: {}, id: uuidv4() },
      ],
    };
  ...
  async function loadSeance(event) {
    ...
    if (data.success === false) {
      ...
      if (data.seance) {
        if (!data.seance.nom) {
          alert(
            'Vous ne pouvez pas charger une séance débutant en mode expert !'
          );
        } else {
        ...
        if (seance.nom) {
          if (seance.nom.ancienNom !== 'nouveau-nom') {
            if (!arr.includes(seance.nom.ancienNom)) {
              ...
              let obj1 = {
                id: 'load',
                label: `Dernière séance "${seance.nom.ancienNom}" enregistrée`,
                value: `lastRec-${seance.nom.ancienNom}`,
              };
              let obj2 = {
                id: 'load',
                label: `Dernière séance "${seance.nom.ancienNom}" en date`,
                value: `lastDate-${seance.nom.ancienNom}`,
              };
            ...
            if (!arr.includes(seance.nom.nouveauNom)) {
              ...
              let obj1 = {
                id: 'load',
                label: `Dernière séance "${seance.nom.nouveauNom}" enregistrée`,
                value: `lastRec-${seance.nom.nouveauNom}`,
              };
              let obj2 = {
                id: 'load',
                label: `Dernière séance "${seance.nom.nouveauNom}" en date`,
                value: `lastDate-${seance.nom.nouveauNom}`,
              };
              if (!containsObj(arr2, obj1)) {
                arr2.push(obj1);
              }
    ...
    <form className="debutant-form">
      <div className=" row">
        <div className="col-12 col-form-label" style={{ marginRight: '10px' }}>
          ...
          <div>
            <button
              className="btn btn-dark loadButton"
              onClick={loadSeance}
              type="submit"
            >
              Charger la séance
      ...
      <div className="NameInput row">
        <div className="col-12 col-form-label" style={{ marginRight: '10px' }}>
          <label className="">Nom de la séance</label>
          <div className="">
            <Select
              placeholder="Nom..."
        ...
        {seance.nom.ancienNom === 'nouveau-nom' ? (
          <div className="form-group row col-12">
            ...
            <input
              type="text"
              className={
                props.modeSombre ? 'form-control inputDark' : 'form-control'
              }
              onChange={handleChangeName}
      ...
      {clickEchauffement ? (
        <div>
          ...
          <button
            className="btn btn-dark form-button"
            onClick={onAddEchauffements}
            type="submit"
          >
            Ajouter un échauffement à cette séance !
          </button>
          <br />
        </div>
      ) : (
        <div>
          <p onClick={handleClickEchauffement} className="expert-title">
            ...
            Echauffement{' '}
            <img
              className="expert-toggle-inverted"
              src={require('../../images/icons/icons8-expand-arrow-90.webp')}
            />{' '}
          </p>
          <hr className={props.modeSombre ? 'hr-serie-dark' : 'hr-serie'} />
      ...
      {clickExercices ? (
        <div>
          <p onClick={handleClickExercices} className="expert-title">
            {' '}
            Exercices{' '}
            <img
              className="expert-toggle"
      ...
      {clickDetails ? (
        <div>
          <p onClick={handleClickDetails} className="expert-title">
            {' '}
            Details{' '}
            <img
              className="expert-toggle"
          ...
          <div className="detail-div">
            ...
            <button
              ...
              type="submit"
            >
              Ajouter un détail à cette séance !
            </button>
          </div>
          <br />
        </div>
      ...
      <div className="form-button-div">
        <button
          className={
            props.modeSombre
              ? 'btn btn-lg btn-black enregistrer-button large-margin-updown'
              : 'btn btn-lg btn-white enregistrer-button large-margin-updown'
          }
          onClick={handleClick}
          type="submit"
        >
