...
function ProgrammeCard(props) {
  ...
  async function handleLike() {
    const { data } = await API.likeProgramme({
  ...
  async function handleComment() {
    const { data } = await API.CommentProgramme({
      programmeId: programme._id,
      userId: localStorage.getItem('id'),
    });
    if (data.success) {
  ...
  async function handleLiked() {
    const { data } = await API.isProgrammeLiked({
      programmeId: programme._id,
      userId: localStorage.getItem('id'),
    });
    if (data.success) {
  ...
  async function handleCommented() {
    const { data } = await API.isProgrammeCommented({
      programmeId: programme._id,
      userId: localStorage.getItem('id'),
    });
    if (data.success) {
  ...
  async function getLikes() {
    const { data } = await API.getProgrammeLikes({
      programmeId: programme._id,
    });
    if (data.success) {
      setLikes(data.likes);
  ...
  async function getProgrammeCreator() {
    const { data } = await API.getProgrammeCreator({
      programmeId: programme._id,
    });
    if (data.success) {
  ...
  async function sendComment() {
    const { data } = await API.sendComment({
      programmeId: programme._id,
      userId: localStorage.getItem('id'),
      comment: comment,
    });
  ...
  async function handleDeleteProgramme() {
    let { data } = await API.deleteProgramme({
      programmeId: programme._id,
  ...
  return (
    <div className="react-card">
      <ReactCardFlip isFlipped={isFlipped} flipDirection="horizontal">
        <div
          className="YOUR_FRONT_CCOMPONENT"
          style={props.modeSombre ? null : { color: 'white' }}
        >
          <div
            className={
              props.modeSombre ? 'programme-card-dark' : 'programme-card'
            }
                ...
                >
                  ...
                  {programme.materiel.map((materiel, index) => {
                    if (materiel.value) {
                      ...
                      if (index === 4) {
                        return (
                          <p style={{ overflow: 'hidden', margin: '0' }}>...</p>
                        );
                      }
                    }
                  })}
                  ...
                  <tr>
                    ...
                    <td>
                      <p style={{ margin: 'auto', display: 'inline-block' }}>
                        <a
                          className="programme-a"
                          href={'/profil?id=' + programmeCreator._id}
                        >
                          {programmeCreator.fName} {programmeCreator.lName}
                        </a>
                      </p>
                      <br />
                      <p style={{ margin: 'auto', display: 'inline-block' }}>
                  ...
                  <tr>
                    ...
                    <td>
                      {commented ? (
                        <img
                          className="small-img"
                          src={require('../../images/icons/comment-red.webp')}
                          onClick={handleFlipCommentTrue}
                          alt="comment"
                        />
                      ) : (
                        <img
                          className="small-img"
                          src={require('../../images/icons/comment.webp')}
                          onClick={handleFlipCommentTrue}
                          alt="comment"
                        />
                      )}
        ...
        <div className="YOUR_BACK_COMPONENT">
            ...
            >
              {whoLikedArray.length > 0 ? (
                ...
                <div style={{ marginTop: '40px' }}>
                  ...
                  <p>
                    Alors attention j'ai pas dis qu'il était nul hein juste que
                    personne l'a liké voilà ça arrive
                  </p>
                  <p>
                    De toute façon je vois pas pourquoi je devrais avoir à me
                    justifier
                  </p>
                </div>
              )}
            ...
            >
              <Scrollbars autoHide>
                  ...
                  {whoCommentedArray.length > 0 ? (
                        ...
                        >
                          ...
                          <tr>
                            ...
                            <td>
                              <div style={{ padding: '5%' }}>
                                <p
                                  onClick={handleFlipCommentFalse}
                                  style={{
                                    margin: 'auto',
                                ...
                                >
                                  <a
                                    className="programme-a"
                                    href={'/profil?id=' + comment.user._id}
                                  >
                                ...
                                <div
                                  className={
                                    dropComment[comment._id]
                                      ? 'dropComment' + ' ' + 'visible'
                                      : 'dropComment' + ' ' + 'not-visible'
                                  }
                    ...
                    <div>
                      <button
                        ...
                        style={
                          props.modeSombre ? null : { borderColor: '#330000' }
                        }
                      >
                        Retour au programme
                      </button>
                      <p>
                        {' '}
                        Tu seras le premier à commenter, c'est un privilège{' '}
                      </p>
                    </div>
                  )}
                </div>
                ...
                <div className="from-group row">
                  <input
                    type="text"
                    className="form-control col-10 comment"
              ...
              <Scrollbars>
                <div>
                  {programme.programme[0].seances ? (
                    <div>
                      {programme.programme.map(
                        (periodisation, indexPeriodisation) => {
                          return (
                            <div>
                              <h1>Periodisation {indexPeriodisation + 1}</h1>
                              {periodisation.seances.map(
                                (seance, indexSeance) => {
                                  return (
                                    <div>
                                      <h2>Seance {indexSeance + 1}</h2>
                                      <div>
                                        {seance.exercices.map(
                                          (exercice, index) => {
                                            return (
                                              <div>
                                                <p>
          ...
          {flipThreeDots ? (
            ...
            >
              <Scrollbars autoHide>
                <button
                  className="btn btn-black large-margin-top mini-margin-bottom"
                  onClick={handleFlipThreeDots}
                >
                ...
                <button
                  className="btn btn-dark basic-margin-updown"
                ...
                <button
                  className="btn btn-dark basic-margin-updown"
                  onClick={handleSignalerProgramme}
                  style={props.modeSombre ? null : { borderColor: '#330000' }}
                >
