...
function verifyToken(req, res) {
  ...
  jwt.verify(token, process.env.secret, function (err, decoded) {
    if (err) {
      res.json({
        success: false,
        message: 'Token verification failed ' + err.name,
      });
    } else {
      res.json({
        success: true,
  ...
  try {
    User.findOneAndUpdate(conditions, update, (err) => {
      if (err) {
        ...
        return res.json({ success: true, message: 'Seance enregistrée !' });
      }
    });
  } catch (e) {
    console.log(e);
        ...
        if (req.query.load) {
          ...
          if (req.query.load[7] === '-') {
            ...
            seances.forEach((seanceIter, index) => {
              if (seanceIter.nom) {
                if (
                  seanceIter.nom.nouveauNom === nomSeance ||
                  seanceIter.nom.ancienNom === nomSeance
                ) {
                  seance = seanceIter;
                }
              }
...
async function priseDeNote(req, res) {
  ...
  function getEchauchements(linesToRow) {
    ...
    echauffementNoteLines.forEach((e, i) => {
      ...
      if (ExoCat.length > 1) {
        ExoCat.forEach((cat, index) => {
          ...
          if (cat.includes('{')) {
            ...
            let utilisation = stringSimilarity.findBestMatch(
              catSplit[0],
              utilisationPossible
            ).bestMatch.target;
      ...
      Serie.forEach((serie, index) => {
        ...
        let percent =
          '' +
          ((parseFloat(charge) / parseFloat(linesToRow[2])) * 100).toFixed(2) +
          '%';
  ...
  try {
    ...
    if (date === 'error') {
      return res.json({
        success: false,
        message: 'Erreur de formalisme de la note (date : jj/mm/aaaa)',
      });
    }
    ...
    if (linesToRow[4] === 'Echauffements:') {
      ...
      if (echauffements === 'error') {
        return res.json({
          success: false,
          message: 'Erreur de formalisme de la note (échauffements)',
        });
      }
    }
    ...
    if (exercices === 'error') {
      return res.json({
        success: false,
        message: 'Erreur de formalisme de la note (exercices)',
      });
    }
    ...
    return res.json({
      success: false,
      message: 'Erreur de formalisme de la note',
    });
  }
}
...
async function getNiveau(req, res) {
  ...
  checkItems['statiqueIntermItem2'] = {
    ...
    description:
      'Tenir une Tuck Planche 20 secondes ou une Advanced Tuck planche 5 secondes.',
    valeur:
      checkPerformance(seances, 'Planche', '', ['Tuck'], 'time', 20, 0, 0) ||
      checkPerformance(
        seances,
        'Planche',
        '',
  ...
  checkItems['statiqueIntermItem3'] = {
    ...
    valeur: checkPerformance(seances, 'L Sit', '', [], 'time', 15, 0),
  };
  ...
  checkItems['pdcIntermItem1'] = {
    ...
    valeur: checkPerformance(seances, 'Dips', '', [], 'reps', 15, 0),
  };
  ...
  checkItems['pdcIntermItem3'] = {
    ...
    valeur: checkPerformance(seances, 'Muscle Up', '', [], 'reps', 3, 0),
  };
  ...
  checkItems['pdcIntermItem5'] = {
    ...
    description: '',
    valeur:
      checkPerformance(seances, 'Squat', '', ['Pistol'], 'reps', 5, 0, 0) ||
      checkPerformance(seances, 'Squat', '', ['Unilatéral'], 'reps', 5, 0),
  };
  ...
  checkItems['streetliftIntermItem1'] = {
    ...
    valeur: checkPerformance(seances, 'Dips', '', [], 'reps', 1, 50),
  };
  ...
  checkItems['streetliftIntermItem4'] = {
    ...
    description: '',
    valeur: checkPerformance(seances, 'Squat', '', [], 1, 130),
  };
  ...
  checkItems['streetliftIntermItem5'] = {
    ...
    description: '',
    valeur: checkPerformance(seances, 'Soulevé de terre', '', [], 1, 170),
  };
  ...
  checkItems['streetliftIntermItem6'] = {
    ...
    description: '',
    valeur: checkPerformance(seances, 'Developpé Couché', '', [], 1, 100),
  };
  ...
  checkItems['streetliftIntermItem7'] = {
    ...
    description: '',
    valeur: checkPerformance(seances, 'Hip Thrust', '', [], 1, 200),
  };
  ...
  checkItems['equilibreIntermItem1'] = {
    ...
    description: '',
    date: new Date(),
    valeur: checkPerformance(seances, 'Handstand', '', [], 'time', 10, 0),
  };
  ...
  checkItems['equilibreIntermItem2'] = {
    ...
    description: '',
    valeur: checkPerformance(seances, 'Handstand', '', [], 'reps', 3, 0),
  };
  ...
  checkItems['equilibreIntermItem3'] = {
    ...
    description: '',
    valeur: checkPerformance(seances, 'Elbow Lever', '', [], 'time', 30, 0),
  };
  ...
  checkItems['statiqueConfirmeItem1'] = {
    ...
    date: new Date(),
    valeur: checkPerformance(seances, 'Front Lever', '', [], 'time', 10, 0),
  };
  ...
  checkItems['statiqueConfirmeItem3'] = {
    ...
    valeur: checkPerformance(seances, 'Drapeau', '', [], 'time', 15, 0),
  };
  ...
  checkItems['pdcConfirmeItem3'] = {
    ...
    valeur: checkPerformance(seances, 'Muscle Up', '', [], 'reps', 8, 0),
  };
  ...
  checkItems['pdcConfirmeItem5'] = {
    ...
    description: '',
    date: new Date(),
    valeur: checkPerformance(seances, 'Squat', '', ['Pistol'], 'reps', 1, 40),
  };
  ...
  checkItems['pdcConfirmeItem6'] = {
    ...
    id: 'pdcConfirmeItem6',
    description: '',
    valeur: checkPerformance(seances, 'Squat', '', ['Matrix'], 'reps', 5, 0),
  };
  ...
  checkItems['streetliftConfirmeItem1'] = {
    ...
    valeur: checkPerformance(seances, 'Dips', '', [], 'reps', 1, 100),
  };
  ...
  checkItems['streetliftConfirmeItem4'] = {
    ...
    description: '',
    valeur: checkPerformance(seances, 'Squat', '', [], 1, 200),
  };
  ...
  checkItems['streetliftConfirmeItem5'] = {
    ...
    description: '',
    valeur: checkPerformance(seances, 'Soulevé de terre', '', [], 1, 250),
  };
  ...
  checkItems['streetliftConfirmeItem6'] = {
    ...
    description: '',
    valeur: checkPerformance(seances, 'Developpé Couché', '', [], 1, 150),
  };
  ...
  checkItems['streetliftConfirmeItem7'] = {
    ...
    description: '',
    valeur: checkPerformance(seances, 'Hip Thrust', '', [], 1, 300),
  };
  ...
  checkItems['equilibreConfirmeItem1'] = {
    ...
    description: '',
    valeur: checkPerformance(seances, 'Handstand', '', [], 'time', 40, 0),
  };
  ...
  checkItems['equilibreConfirmeItem2'] = {
    ...
    description: '',
    valeur: checkPerformance(seances, 'Handstand', '', [], 'reps', 5, 0),
  };
  ...
  checkItems['statiqueExpertItem1'] = {
    ...
    description:
      'Tenir un Front Lever pendant 20 secondes ou un Front Lever à une main',
    date: new Date(),
    valeur:
      checkPerformance(seances, 'Front Lever', '', [], 'time', 20, 0) ||
      checkPerformance(
        seances,
        'Front Lever',
        '',
  ...
  checkItems['statiqueExpertItem2'] = {
    ...
    valeur:
      checkPerformance(seances, 'Planche', '', [], 'time', 10, 0, 0) ||
      checkPerformance(seances, 'Maltese', '', [], 'time', 10, 0),
  };
  ...
  checkItems['statiqueExpertItem3'] = {
    ...
    valeur: checkPerformance(seances, 'Iron Cross', '', [], 'time', 10, 0),
  };
  ...
  checkItems['statiqueExpertItem4'] = {
    ...
    valeur: checkPerformance(seances, 'Drapeau', '', [], 'time', 30, 0),
  };
  ...
  checkItems['pdcExpertItem1'] = {
    ...
    valeur: checkPerformance(seances, 'Dips', '', ['Unilatéral'], 'reps', 5, 0),
  };
  ...
  checkItems['pdcExpertItem3'] = {
    ...
    valeur: checkPerformance(seances, 'Muscle Up', '', [], 'reps', 12, 0),
  };
  ...
  checkItems['pdcExpertItem4'] = {
    ...
    id: 'pdcExpertItem4',
    description: '',
    valeur: checkPerformance(seances, 'Planche', '', [], 'reps', 5, 0),
  };
  ...
  checkItems['pdcExpertItem5'] = {
    ...
    id: 'pdcExpertItem5',
    description: '',
    valeur: checkPerformance(seances, 'Squat', '', ['Pistol'], 'reps', 1, 60),
  };
  ...
  checkItems['pdcExpertItem6'] = {
    ...
    id: 'pdcExpertItem6',
    description: '',
    valeur: checkPerformance(seances, 'Squat', '', ['Matrix'], 'reps', 10, 0),
  };
  ...
  checkItems['streetliftExpertItem1'] = {
    ...
    valeur: checkPerformance(seances, 'Dips', '', [], 'reps', 1, 130),
  };
  ...
  checkItems['streetliftExpertItem4'] = {
    ...
    description: '',
    valeur: checkPerformance(seances, 'Squat', '', [], 1, 230),
  };
  ...
  checkItems['streetliftExpertItem5'] = {
    ...
    description: '',
    valeur: checkPerformance(seances, 'Soulevé de terre', '', [], 1, 300),
  };
  ...
  checkItems['streetliftExpertItem6'] = {
    ...
    description: '',
    date: new Date(),
    valeur: checkPerformance(seances, 'Developpé Couché', '', [], 1, 200),
  };
  ...
  checkItems['streetliftExpertItem7'] = {
    ...
    description: '',
    date: new Date(),
    valeur: checkPerformance(seances, 'Hip Thrust', '', [], 1, 400),
  };
  ...
  checkItems['equilibreExpertItem1'] = {
    ...
    description: '',
    valeur: checkPerformance(seances, 'Handstand', '', [], 'time', 90, 0),
  };
  ...
  checkItems['equilibreExpertItem2'] = {
    ...
    description: '',
    valeur: checkPerformance(seances, 'Handstand', '', [], 'reps', 10, 0),
  };
  ...
  if (Object.values(userCheckItems).length > 0) {
    Object.values(checkItems).forEach((item, index) => {
      ...
      if (
        Object.values(userCheckItems)[index].valeur === true &&
        item.valeur === false
      ) {
...
async function workouts(req, res) {
  ...
  function seanceChargeSort(b, a) {
    const A = parseFloat(
      a.exercices.map((exercice, indexExercice) => {
    ...
    const B = parseFloat(
      b.exercices.map((exercice, indexExercice) => {
        return Object.values(exercice.Series).map((serie, index) => {
          return serie.charge;
        });
      })
        ...
        return Object.keys(exercice.Series).map((index) => {
          ...
          if (multiple) {
            seances[indexSeance].exercices[indexExercice].Series[
              ...
              seances[indexSeance].exercices[indexExercice].Series[index]
                .repsTime * multiple;
          }
          let obj = {};
              ...
              exercices: [
                {
                  ...
                  Series: {
                    0: seances[indexSeance].exercices[indexExercice].Series[
                      index
                    ],
                  },
                  Categories:
                    seances[indexSeance].exercices[indexExercice].Categories,
                },
              ],
            };
          } else {
  ...
  function removeEmpty(seance) {
    return Object.fromEntries(
      Object.entries(seance).filter(([_, element]) => {
        if (element[0]) {
          if (typeof element !== 'string') {
            if (element[0].Series) {
              return (
                element[0].Series != null &&
                Object.entries(element[0].Series).length !== 0
              );
            }
          } else {
  ...
  function seancesToPie(seances, string) {
      ...
      return seance.exercices.map((exercice, indexExercice) => {
        return Object.keys(exercice.Series).map((index) => {
          if (
            ...
            if (
              seances[indexSeance].exercices[indexExercice].exercice.ownExercice
            ) {
              namesList.push(
                seances[indexSeance].exercices[indexExercice].exercice
                  .ownExercice
              );
              if (string !== 'sets') {
                sumsList.push(
    ...
    //sets
      ...
      for (let k = 0; k < namesList.length; k++) {
        namesSumsObj.push({
          name: uniqueNamesList[k],
          repsTime: parseFloat(sumsList[k]),
        });
      }
  ...
  try {
    User.find(isAdmin(req.query), function (err, data) {
      ...
      if (isAdmin(req.query) === { _id: req.query.id }) {
        ...
        seances.forEach((seance, index) => {
          ...
          seance.exercices.forEach((exercice, indexEx) => {
            if (
              exercice.exercice.ownExercice !== '' &&
              !ownExercices.includes(exercice.exercice.ownExercice)
            ) {
        ...
        seances.map((seance, indexSeance) => {
          ...
          if (seance.nom) {
            if (
              ...
              seance.nom.nouveauNom !== req.query.nom
            ) {
      ...
      seances.map((seance, indexSeance) => {
        return seance.exercices.map((exercice, indexExercice) => {
          if (
            req.query.exerciceName !== 'title' &&
            req.query.exerciceName !== ''
          ) {
            if (req.query.exerciceName !== 'own-exercice') {
              if (
                req.query.exerciceMuscle !== '' &&
                req.query.exerciceMuscle !== 'title'
              ) {
                if (
                  req.query.exerciceName !== exercice.exercice.name ||
                  req.query.exerciceMuscle !== exercice.exercice.muscle
                ) {
                  delete seances[indexSeance].exercices[indexExercice];
                }
              } else {
              ...
              if (
                req.query.exerciceOwnExercice !== exercice.exercice.ownExercice
              ) {
                delete seances[indexSeance].exercices[indexExercice];
              }
            }
      ...
      if (req.query.categorie0name === 'Aucune') {
        seances.map((seance, indexSeance) => {
          return seance.exercices.map((exercice, indexExercice) => {
            if (
              exercice.Categories &&
              Object.entries(exercice.Categories).length !== 0
            ) {
              ...
              seances.map((seance, indexSeance) => {
                return seance.exercices.map((exercice, indexExercice) => {
                  if (
                    ...
                    del = true;
                    Object.values(exercice.Categories).map(
                      (categorie, indexCategorie) => {
                        if (
                          categorie.name === req.query[catName] &&
                          categorie.input === req.query[catInput]
                        ) {
                          del = false;
                        }
                      }
                  ...
                  if (
                    ...
                    Object.values(exercice.Categories).map(
                      (categorie, indexCategorie) => {
                        if (
                          categorie.name === req.query[catName] &&
                          categorie.utilisation === req.query[catUtilisation]
                        ) {
                          del = false;
                        }
                      }
        ...
        for (let i = 0; i < 5; i++) {
          ...
          if (
            ...
            seances.map((seance, indexSeance) => {
              if (
                seance.details &&
                Object.entries(seance.details).length !== 0
              ) {
                del = true;
                seance.details.map((detail, indexDetail) => {
                  if (
                    detail.name === req.query[catName] &&
                    detail.input === req.query[catInput]
                  ) {
                    del = false;
                  }
                });
          ...
          return seance.exercices.map((exercice, indexExercice) => {
            return Object.values(exercice.Series).map((serie, index) => {
              if (parseFloat(serie.repsTime) < req.query.repsFrom) {
                delete seances[indexSeance].exercices[indexExercice].Series[
                  index
                ];
              }
            });
      ...
      if (req.query.reforme === 'true') {
        ...
        let arr2 = [];
        seances.forEach((seance) => {
          seance.exercices[0].Series[0].percent = parseFloat(
            seance.exercices[0].Series[0].percent
          );
        ...
        seances.forEach((seance) => {
          for (let k = 0; k < 5; k++) {
            if (
              ...
              seance.exercices[0].Categories[k] &&
              seance.exercices[0].Categories[k].estimation
            ) {
              seance.exercices[0].Categories[k].estimation = parseFloat(
                seance.exercices[0].Categories[k].estimation
              );
              if (
                seance.exercices[0].Categories[k].utilisation === 'Resistance'
              ) {
                seance.exercices[0].Categories[k].resistance = parseFloat(
                  seance.exercices[0].Categories[k].estimation
                );
              }
              if (
                seance.exercices[0].Categories[k].utilisation === 'Assistance'
              ) {
                seance.exercices[0].Categories[k].assistance = parseFloat(
                  seance.exercices[0].Categories[k].estimation
                );
              }
            }
      ...
      if (req.query.date === 'd') {
        seances.forEach((seance) => {
          seance.date = seance.date.slice(
            seance.date.length - 2,
            seance.date.length
          );
        });
      }
      ...
      if (req.query.reforme === 'poids') {
        ...
        arr = [];
        seances.forEach((seance) => {
          arr.push(parseFloat(seance.poids));
        });
      ...
      res.json({
        ...
        message: 'Utilisateur trouvé !',
        seances: seances,
        numSeanceDay: numSeanceDay,
        numUsers: numUsers,
        numSeances: numSeances,
        numActiveUsers: numActiveUsers,
        ownExercices: ownExercices,
        poidsMax: poidsMax,
        poidsMin: poidsMin,
        chargeMax: chargeMax,
...
async function getUser(req, res) {
  ...
  try {
    User.find(conditions, function (err, data) {
      if (err) {
        res.json({ success: false, message: err });
      } else {
        if (data.length === 0) {
...
async function reguScore(req, res) {
  ...
  if (seances.length > 1) {
    ...
    for (let k = 0; k < weekAndYear.length - 1; k++) {
      ...
      if (
        (S2.week === S1.week + 1 || S2.week === S1.week) &&
        S2.year === S1.year
      ) {
        consecutiveSeances++;
        bool = true;
    ...
    let score =
      ((seancesOnWeeks +
        average(consecutivePeriods) / weeksOverPeriod +
    ...
    res.json({
      success: true,
      message: 'Seances trouvées',
      reguScore: reguScore,
      bestSerie: Math.max(...consecutivePeriods),
